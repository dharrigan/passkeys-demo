...
;;
(def ^:private main 'passkeys.demo.main)

;;
;; Should be no need to change anything below...
;;
(def ^:private class-dir "target/classes")
(def ^:private config-dir "target/classes/config")
...
(defn uberjar
  ...
  (let [opts' (uber-opts opts)]
    ...
    ;;
    ;; Delete the config directory, as we'll supply that at the start of
    ;; the application in the classpath, i.e., -cp "config:app.jar"
    ;;
    ;; Also prevents config leakage into the UberJAR.
    ;;
    (b/delete {:path config-dir})
    ;;
    ;;
    ;;
